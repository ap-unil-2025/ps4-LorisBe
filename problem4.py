...
def backup_file(source_filename, backup_filename):
    """
    Create a backup copy of a JSON file by loading and re-saving.
    """
...
def merge_json_files(file1, file2, output_file):
    ...
    Treats missing/invalid files as empty lists.
    """
    data1 = load_from_json(file1)
    data2 = load_from_json(file2)

    list1 = data1 if isinstance(data1, list) else []
    list2 = data2 if isinstance(data2, list) else []
...
def search_json_file(filename, key, value):
    """
    Search a JSON file (list of dicts) for items where item[key] == value.
    """
    data = load_from_json(filename)
